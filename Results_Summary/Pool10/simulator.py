...
if __name__ == '__main__':
	...
	pool_num = 10
	teams = [18, 28, 43, 45, 61, 63, 64, 68]
	scores = {}

	dir_name = "Pool" + str(pool_num) + "/"
	os.mkdir(dir_name)

	for i in range(len(teams)):
		for j in range(len(teams)):
			if i!=j:
				module1 = "team"+str(teams[i])
				class1 = "Team"+str(teams[i])
				module2 = "team"+str(teams[j])
				class2 = "Team"+str(teams[j])
				obj1 = getattr(__import__(module1), class1)()
				obj2 = getattr(__import__(module2), class2)()
				sys.stdout = open(dir_name+module1+"vs"+module2, "w")
				print "##################################### START OF", module1.upper(), "VS.", module2.upper(), "#####################################\n"
				print class1, "(P1,x) is going first"
				print class2, "(P2,0) is going second\n"
				x = gameplay(obj1, obj2)
				print "###################################### END OF", module1.upper(), "VS.", module2.upper(), "######################################\n"
				print "Player1", "("+class1+")", "points:", x[0]
				print "Player2", "("+class2+")", "points:", x[1]
				if class1 in scores:
					scores[class1] += x[0]
				else:
					scores[class1] = x[0]
	...
	sys.stdout = open(dir_name + "pool"+str(pool_num)+"_points_summary", "w")
	score_list = sorted(scores.items(), key = lambda kv:(kv[1], kv[0]), reverse = True)
	for score in score_list:
		print score[0]+":", score[1]

	# sys.stdout = sys.__stdout__ # to set stdout back to original
